...
def initialize_db():
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS user_auto_save_status (
            user_id INTEGER PRIMARY KEY,
...
def get_auto_save_status(user_id):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute('SELECT auto_save_enabled FROM user_auto_save_status WHERE user_id = ?', (user_id,))
    row = cursor.fetchone()
    conn.close()
    return bool(row['auto_save_enabled']) if row else False

def save_auto_save_status(user_id, status):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute('''
        INSERT OR REPLACE INTO user_auto_save_status (user_id, auto_save_enabled)
        VALUES (?, ?)
    ''', (user_id, status))
    conn.commit()
    conn.close()
...
def get_broadcasting_status(user_id):
    ...
    if row:
        return {
            ...
            "target_chats_data": json.loads(row['target_chats_data']) if row['target_chats_data'] else None,
        ...
        return {
            ...
            "broadcast_type": None,
            "sleep_time": None,
            "target_chats_data": None,
            "message_id": None,
            "chat_id_of_message": None
        }

def save_broadcasting_status(user_id, status_data):
    conn = get_db_connection()
    cursor = conn.cursor()
    is_broadcasting = status_data.get("is_broadcasting", False)
    broadcast_type = status_data.get("broadcast_type")
    sleep_time = status_data.get("sleep_time")
    target_chats_data = json.dumps(status_data.get("target_chats_data")) if status_data.get("target_chats_data") is not None else None
    message_id = status_data.get("message_id")
    chat_id_of_message = status_data.get("chat_id_of_message")

    cursor.execute('''
        INSERT OR REPLACE INTO user_broadcasting_status (user_id, is_broadcasting, broadcast_type, sleep_time, target_chats_data, message_id, chat_id_of_message)
        VALUES (?, ?, ?, ?, ?, ?, ?)
    ''', (user_id, is_broadcasting, broadcast_type, sleep_time, target_chats_data, message_id, chat_id_of_message))
    conn.commit()
    conn.close()
